...
type CampaignGetRequest struct {
	...
	Filtering       *CampaignGetFiltering    `json:"filtering,omitempty"`        // 筛选字段
...
type CampaignGetFiltering struct {
	CampaignIds  []uint64            `json:"campaign_ids,omitempty"`  // 广告组 id 列表
	CampaignName string              `json:"campaign_name,omitempty"` // 广告组名
...
type CampaignGetResponse struct {
	model.BaseResponse
	Data *CampaignGetResponseData `json:"data,omitempty"`
}

type CampaignGetResponseData struct {
	List     []CampaignGetResponseItem `json:"list,omitempty"`
	PageInfo *model.PageInfo           `json:"page_info,omitempty"`
}

type CampaignGetResponseItem struct {
...
func (r CampaignGetRequest) Encode() string {
	values := &url.Values{}
